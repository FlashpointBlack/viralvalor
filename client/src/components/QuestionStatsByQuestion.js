...
// Helper to convert milliseconds to a human-readable string
...
// Admin-only component that lists statistics aggregated by individual question
const QuestionStatsByQuestion = () => {
  const { user } = useAuth0();
  ...
  const fetchData = () => {
    ...
    axios
      .get('questions/admin/question-stats-by-question', { headers: { 'x-user-sub': userSub }, params })
  ...
  const applyPreset = (p) => {
    ...
    switch (p) {
      ...
      case 'custom':
      default:
        // For custom we wait until both dates selected
        break;
    }
  };

  // Fetch when preset or dates change, respecting custom rule
  useEffect(() => {
    if (
      preset !== 'custom' ||
  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '1rem' }}>
        <ExportPdfButton 
          targetRef={reportRef}
          filename="question-practice-report.pdf"
          preProcess={(doc) => {
            ...
            container.querySelectorAll('.qp-report-table').forEach(tbl => {
              ...
              // carry over rowSpan cells from first row
              ...
              // clear thead and append single row
              thead.innerHTML = '';
            ...
            header.textContent = `Question Practice Report — For period inclusive of: ${periodText}`;
      ...
      <div ref={reportRef} className="question-practice-container">
        <h2 style={{ textAlign: 'center', marginBottom: '1rem' }}>Question Practice Reports</h2>
        ...
        {rows.length === 0 ? (
          ...
          <table className="qp-report-table">
            <thead>
              <tr>
                {/* Primary label row with grouped metrics */}
                <th rowSpan="2" className="sortable" onClick={() => handleSort('questionText')}>
                  Question{sortIndicator('questionText')}
                </th>
                <th rowSpan="2" className="sortable" onClick={() => handleSort('distinctUsers')}>
                  Unique Users{sortIndicator('distinctUsers')}
                </th>
                <th rowSpan="2" className="sortable" onClick={() => handleSort('totalAttempts')}>
                  Total Attempts{sortIndicator('totalAttempts')}
                </th>
                {/* Group labels */}
              ...
              <tr>
                {/* Added Correct/Incorrect under Accuracy */}
                ...
                {/* Sub-headers underneath groups: accuracy metrics first per order */}
            ...
            <tbody>
              {sortedRows.map((r) => {
                ...
                return (
                  <tr key={r.questionId}>
                    <td style={{ textAlign: 'left' }}>{r.questionText}</td>
                    <td>{r.distinctUsers}</td>
                    ...
                    {/* Accuracy details */}
                    <td>
                      {lowestAccPct !== 'NaN' ? `${lowestAccPct}%` : '—'}{r.lowestAccUser ? ` (${r.lowestAccUser})` : ''}
                    </td>
                    <td>{avgAccPct !== 'NaN' ? `${avgAccPct}%` : '—'}</td>
                    <td>
                      {highestAccPct !== 'NaN' ? `${highestAccPct}%` : '—'}{r.highestAccUser ? ` (${r.highestAccUser})` : ''}
                    </td>

                    {/* Timing metrics */}
                    <td>{msToReadable(r.slowestTimeMs)}{r.slowestUser ? ` (${r.slowestUser})` : ''}</td>
                    <td>{msToReadable(r.avgTimeMs)}</td>
                    <td>{msToReadable(r.fastestTimeMs)}{r.fastestUser ? ` (${r.fastestUser})` : ''}</td>
                  </tr>
                );
              })}
            </tbody>
...
export default QuestionStatsByQuestion; 