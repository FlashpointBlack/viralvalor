...
export const AuthProvider = ({ children }) => {
  ...
  const fetchAuthUserData = useCallback(async () => {
    if (isAuthenticated && user?.sub && token !== null) {
      ...
      try {
        ...
        const { data: profileData } = await axios.get('/api/user/profile-status', authHeader);
        setProfileComplete(profileData.profileComplete);
        setIsAdmin(profileData.isAdmin);

        const { data: meData } = await axios.get('/api/user/me', authHeader);
        setUserData(meData);
  ...
  useEffect(() => {
    if (user?.sub && !userData) {
      ...
      axios.get(`/api/user/by-sub/${encodeURIComponent(user.sub)}`)
        .then(({ data }) => data)
        .then(data => {
          setUserData(data);
