...
/**
 * Registers all lecture-related REST endpoints on the provided Express app.
 * Mirrors the behaviour that previously lived directly in routes.js but moved
 * here to keep that file manageable.
 *
 * NO PUBLIC API CHANGES – existing paths continue to function the same.
 *
 * @param {import('express').Express} app
 */
function setupLectureRoutes(app) {
  // Upload middleware for *.pptx etc.
  const lectureUpload = setupMulter('public/lectures/uploads').single('file');

  // -----------------------------------------------------------------------
  //  CRUD + Ancillary lecture endpoints
  // -----------------------------------------------------------------------
  ...
  // [POST] /upload-lecture-file  – creates lecture & file in one step
  ...
  // [POST] /upload-lecture-file-existing
  ...
  app.post('/delete-lecture', async (req, res) => {
    ...
    try {
      ...
      // Remove lecture access rows first. These may not exist in older schemas or
      // may use a slightly different column naming convention. Any failure here
      // should **never** block the primary lecture deletion flow.
      try {
        ...
        // Intentionally swallow the error so the overall deletion can continue.
      }

      // Cascade delete questions + ancillary data
  ...
  // [GET] /my-released-lectures – for students
  ...
  // [POST] /submit-lecture-for-approval (educator)
  ...
  // -----------------------------------------------------------------------
  app.get('/lectures/health', (req, res) => {
    res.json({ status: 'ok' });
  });
...
module.exports = { setupLectureRoutes }; 